  ...
  public function athleteProfile($id) {
	...
	public static function get2019() {
		...
		foreach($divs as $key => $div) {
			...
			$registrations[$key]['Men']['Registrants']['Rx'] = $result['pagination']['totalCompetitors'];
			...
			$url = $base . '?' . http_build_query($args);
			$result = AthleteController::getUrl($url);
			$registrations[$key]['Women']['Registrants']['Rx'] = $result['pagination']['totalCompetitors'];
			...
			$url = $base . '?' . http_build_query($args);
			$result = AthleteController::getUrl($url);
			$registrations[$key]['Men']['Registrants']['Scaled'] = $result['pagination']['totalCompetitors'];
			...
			$url = $base . '?' . http_build_query($args);
			$result = AthleteController::getUrl($url);
			$registrations[$key]['Women']['Registrants']['Scaled'] = $result['pagination']['totalCompetitors']; */
		}
		...
		foreach($registrations as $div => $d) {
			$totalMen += $d['Men']['Registrants']['Rx'];
			$totalWomen += $d['Women']['Registrants']['Rx'];
		}

		$registrations['Total']['Men']['Registrants']['Rx'] = $totalMen;
		$registrations['Total']['Women']['Registrants']['Rx'] = $totalWomen;

		$totalMen = 0;
		$totalWomen  = 0;

		foreach($registrations as $div => $d) {
			$totalMen += $d['Men']['Registrants']['Scaled'];
			$totalWomen += $d['Women']['Registrants']['Scaled'];
		}

		$registrations['Total']['Men']['Registrants']['Scaled'] = $totalMen;
		$registrations['Total']['Women']['Registrants']['Scaled'] = $totalWomen; */
	...
	public static function get2017() {
		$base = 'https://games.crossfit.com/competitions/api/v1/competitions/open/2017/leaderboards';
		...
		foreach($divs as $key => $div) {
			...
			$url = $base . '?' . http_build_query($args);
			$result = AthleteController::getUrl($url);
			$registrations[$key]['Men']['Registrants']['Rx'] = count($result['athletes']) + (50*($pages-1));
			...
			$url = $base . '?' . http_build_query($args);
			$result = AthleteController::getUrl($url);
			$registrations[$key]['Women']['Registrants']['Rx'] = count($result['athletes']) + (50*($pages-1));
			...
			$url = $base . '?' . http_build_query($args);
			$result = AthleteController::getUrl($url);
			$registrations[$key]['Men']['Registrants']['Scaled'] = count($result['athletes']) + (50*($pages-1));
			...
			$url = $base . '?' . http_build_query($args);
			$result = AthleteController::getUrl($url);
			$registrations[$key]['Women']['Registrants']['Scaled'] = count($result['athletes']) + (50*($pages-1)); */
		}
